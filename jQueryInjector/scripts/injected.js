...
function safe_inject() {
	...
	if (options["deleteOtherReferences"] == true && document.scripts != null && document.scripts.length > 0) {
		/*!
		 * Since document.scripts represents a live list of items, we have to traverse it in
...
chrome.runtime.onMessage.addListener(function (message, sender, callback) {
	if (message.function == "inject") {
		...
		chrome.runtime.sendMessage({
			jqueryPresent: Array.from(document.scripts).filter(script => script.src.includes("jquery")).length > 0
		});
	}
});
